    ...
    <div class="card">
                ...
                <div class="col-md-6">
                    ...
                    <select wire:model="selectedNGReport" class="form-select">
                        ...
                        @foreach($ngReports as $report)
                            <option value="{{ $report->id }}">{{ $report->date }} - {{ $report->ng_type }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select wire:model="selectedQualityCheck" class="form-select">
                        ...
                        @foreach($qualityChecks as $check)
                            <option value="{{ $check->id }}">{{ $check->check_time }} - {{ $check->status }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="card-body">
                    ...
                    <div class="card mb-3">
                        <div class="card-header bg-light">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <button wire:click="addCause('{{ $category }}')" class="btn btn-primary btn-sm">
                        ...
                        <div class="card-body">
                            @foreach($categoryCauses as $index => $cause)
                                <div class="row mb-3 align-items-center">
                                    <div class="col-md-3">
                                        <input type="text" wire:model="causes.{{ $category }}.{{ $index }}.cause" 
                                               class="form-control" placeholder="Cause">
                                    </div>
                                    <div class="col-md-4">
                                        <input type="text" wire:model="causes.{{ $category }}.{{ $index }}.description" 
                                               class="form-control" placeholder="Description">
                                    </div>
                                    <div class="col-md-2">
                                        <input type="text" wire:model="causes.{{ $category }}.{{ $index }}.pic" 
                                               class="form-control" placeholder="PIC">
                                    </div>
                                    <div class="col-md-2">
                                        <input type="date" wire:model="causes.{{ $category }}.{{ $index }}.target_date" 
                                               class="form-control">
                                    </div>
                                    <div class="col-md-1">
                                        <button wire:click="removeCause('{{ $category }}', {{ $index }})" 
            ...
            <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
                ...
                <button wire:click="save" class="btn btn-success" {{ !$showDiagram ? 'disabled' : '' }}>
                    <i class="bi bi-save"></i> {{ $editMode ? 'Update' : 'Save' }} Analysis
                </button>
            </div>
                    ...
                    <tbody>
                        @foreach($analyses as $analysis)
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-{{ $analysis->status === 'draft' ? 'warning' : 
                                ...
                                <td class="text-nowrap">
                                    <div class="btn-group" role="group">
                                        <button wire:click="viewDiagram({{ $analysis->id }})" 
                                                class="btn btn-sm btn-info" 
                                                title="View Diagram">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                
                                        <button wire:click="viewDetail({{ $analysis->id }})" 
                                                class="btn btn-sm btn-primary" 
                                                title="View Details">
                                            <i class="bi bi-info-circle"></i>
                                        </button>
                                
                                        <button wire:click="editAnalysis({{ $analysis->id }})" 
                                                class="btn btn-sm btn-warning" 
                                                title="Edit Analysis">
                                            <i class="bi bi-pencil"></i>
                                        </button>
                                
                                        <a href="{{ route('fishbone.pdf', $analysis->id) }}" 
                                           target="_blank" 
                                        ...
                                        @if($analysis->status === 'draft')
                                            <button wire:click="updateStatus({{ $analysis->id }}, 'in_progress')" 
                 ...
                 <div class="modal-body">
                     @if($selectedAnalysis)
                         ...
                         <div class="mb-4">
                             ...
                             @if($selectedAnalysis->cloudinary_url)
                                 <div class="text-center">
                                     <img src="{{ $selectedAnalysis->cloudinary_url }}" 
                         ...
                         <div class="mb-3">
                             ...
                             <p>{{ $selectedAnalysis->problem_statement }}</p>
                         </div>
                         ...
                         <div class="mb-3">
                             <h6>Root Causes</h6>
                             <div class="row">
                                 @foreach(['Man', 'Machine', 'Method', 'Material', 'Measurement', 'Environment'] as $category)
                                     <div class="col-md-6 mb-3">
                                         <div class="card">
                                             <div class="card-header">{{ $category }}</div>
                                             <div class="card-body">
                                                 <ul class="list-unstyled">
                                                     @forelse($selectedAnalysis->causes->where('category', $category) as $cause)
                                                         <li class="mb-2">
                                                             <strong>{{ $cause->cause }}</strong>
                                                             @if($cause->description)
                                                                 <br>
                                                                 <small class="text-muted">Description: {{ $cause->description }}</small>
                                                             @endif
                                                             @if($cause->pic)
                                                                 <br>
                                                                 <small class="text-muted">PIC: {{ $cause->pic }}</small>
                                                             @endif
                                                             @if($cause->target_date)
                                                                 <br>
                                                                 <small class="text-muted">Target: {{ $cause->target_date }}</small>
                                                             @endif
                                                         </li>
                                                     @empty
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    @if($selectedAnalysis && $selectedAnalysis->cloudinary_url)
                        <img src="{{ $selectedAnalysis->cloudinary_url }}" 
                             class="img-fluid rounded shadow" 
                             alt="Fishbone Diagram"
                             style="max-width: 100%; height: auto;">
